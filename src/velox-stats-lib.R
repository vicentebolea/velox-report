...
waves_plot <- function(waves) {
  ...
  p <- ggplot(data=waves) +
    geom_rect(aes(ymin=idx - 0.2, ymax=idx+ 0.2, xmin=min_time, xmax=max_time, fill=dfs),color = "black") +
    geom_label(aes(x = (max_time +min_time)/2, y = idx, label=l), fill="transparent", color="white") +
